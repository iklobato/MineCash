...
- Terraform >= 1.0
- AWS Provider >= 6.0 (v6.x) - Required for ECS deployment configuration compatibility
...
2. **Configure variables**:
   ...
   # Note: project_name defaults to "minecraft" for backward compatibility
   # You can customize it for different projects (e.g., "myproject")
   ```
...
- `project_name`: Project name used for resource naming (required, e.g., "minecraft")
- `environment`: Environment name used for resource naming and tagging (required, e.g., "production")
...
- `cluster_id`: Redis cluster identifier (required, typically `{project_name}-redis-{environment}`)
- `node_type`: Redis node instance type (default: "cache.t3.micro")
- `num_cache_nodes`: Number of cache nodes (default: 2)
- `subnet_group_name`: ElastiCache subnet group name (required, typically `{project_name}-redis-subnet-{environment}`)
...
1. **CloudWatch Log Group** (`aws_cloudwatch_log_group.main`):
   - **Name**: `/ecs/{project_name}-server` (e.g., `/ecs/minecraft-server`)
...
- **Container Name**: `{project_name}-server` (e.g., `minecraft-server`)
...
- **Log Configuration**: CloudWatch Logs driver, log group `/ecs/{project_name}-server` (e.g., `/ecs/minecraft-server`)
...
- `cluster_name`: ECS cluster name (required, typically `{project_name}-cluster-{environment}`)
- `service_name`: ECS service name (required, typically `{project_name}-server-{environment}`)
...
- `Project`: "{project_name}-server" (e.g., "minecraft-server", auto-generated from project_name variable)
...
module "vpc" {
  ...
  project_name        = "minecraft"             # Required (defaults to "minecraft" if not specified)
  environment         = "production"            # Required (defaults to "production" if not specified)
  tags                = {}                     # Optional
}
...
3. **Resource Naming**:
   - Use descriptive names: `{project_name}-{component}-{identifier}` (e.g., `minecraft-vpc`, `myproject-alb`)
   - Include environment in names when appropriate (e.g., `{project_name}-cluster-{environment}`)
   - Use consistent naming patterns across modules
   - All modules use `project_name` variable for consistent naming (defaults to "minecraft" for backward compatibility)
